...
def create_modern_button_style(theme_colors, button_type='primary'):
    """创建现代化按钮样式
    
    Args:
        theme_colors (dict): 主题颜色配置
        button_type (str): 按钮类型 ('primary', 'success', 'info', 'warning', 'danger')
        
    Returns:
        str: CSS样式字符串
    """
    color_map = {
        'primary': theme_colors.get('primary', '#007ACC'),
        'success': theme_colors.get('success', '#10B981'),
        'info': theme_colors.get('info', '#3B82F6'),
    ...
    return f"""
        ...
        padding: {UI_SPACING['small']} {UI_SPACING['large']};
        background: {bg_color};
        border-radius: {UI_BORDER_RADIUS['small']};
        font-size: {UI_FONT_SIZES['small']};
        font-weight: 500;
        box-shadow: {UI_SHADOWS['subtle']};
        margin-right: {UI_SPACING['normal']};
