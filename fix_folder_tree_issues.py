...
def generate_folder_filter_fix():
    """生成修复文件夹过滤问题的代码"""
...
def main():
    ...
    # 生成修复代码
    folder_tree_fix = generate_folder_tree_fix()
    folder_filter_fix = generate_folder_filter_fix()
    statistics_fix = generate_statistics_fix()
...
{normalize_path_for_display.__doc__}
def normalize_path_for_display(path_str):
    """用于显示的路径标准化函数"""
    if not path_str:
        return ""
...
'''
    ...
    # 保存修复代码到文件
    with open('folder_tree_fix_code.py', 'w', encoding='utf-8') as f:
        f.write(complete_fix)
    
    print("✅ 修复代码已生成到 folder_tree_fix_code.py")
    print("\n问题总结:")
    print("1. 路径大小写不一致：D:\\ 和 d:\\ 同时出现")
    print("2. 文件夹统计数量不匹配：各文件夹数量之和不等于总数")
    print("3. 文件夹过滤逻辑存在问题")
    
    print("\n解决方案:")
    print("1. 添加统一的路径标准化函数")
    print("2. 修复文件夹树构建逻辑，避免重复路径")
    print("3. 修复文件夹过滤和统计逻辑")
    print("4. 添加调试功能以便排查问题")
    
    print("\n下一步:")
    print("1. 将生成的修复代码集成到 search_gui_pyside.py")
    print("2. 测试修复效果")
    print("3. 验证文件夹统计数量是否正确")
