...
class MainWindow(QMainWindow):  # Changed base class to QMainWindow
    ...
    def _create_action_buttons(self):
        ...
        # 索引目录按钮（新增）
        self.index_dirs_button = QPushButton("索引目录")
        self.index_dirs_button.setToolTip("查看并选择当前索引的文件夹")
        self.index_dirs_button.setMinimumWidth(100)  # 设置最小宽度确保按钮足够宽
        self.index_dirs_button.setObjectName("index_button")  # 使用相同的objectName来应用相同样式
        ...
        # 将按钮添加到布局
        action_layout.addWidget(self.index_dirs_button)
    ...
    def _setup_connections(self):
        ...
        # --- 添加索引按钮的信号连接 ---
        self.index_dirs_button.clicked.connect(self.show_index_directories_slot)
    ...
    def show_license_dialog_slot(self):
        ...
        # 无论对话框是接受还是拒绝，都手动更新一次菜单状态和功能
        # 确保在许可证状态改变后立即更新菜单显示
        
        # 保存当前主题设置
        current_theme = self.settings.value("ui/theme", "系统默认")
        
        # 检查主题是否需要变更（如果当前是专业版主题但许可证已失效）
        advanced_themes_available = self.license_manager.is_feature_available(Features.ADVANCED_THEMES)
        if not advanced_themes_available and current_theme not in ["系统默认", "现代蓝"]:
            # 强制切换到现代蓝主题
        ...
        # 强制刷新主题图标
        if current_theme == "现代蓝":
            icon_path = get_resource_path("down_arrow_blue.png")
        elif current_theme == "现代绿":
            icon_path = get_resource_path("down_arrow_green.png")
        elif current_theme == "现代紫":
            icon_path = get_resource_path("down_arrow_purple.png")
        else:
            icon_path = get_resource_path("down_arrow_blue.png")
            
        # 强制应用图标到所有下拉框
        if os.path.exists(icon_path):
            self._apply_direct_arrow_icons(icon_path)
            print(f"许可证状态变更后，强制更新下拉框图标: {icon_path}")
            
            # 更新主题图标设置
            self._update_theme_icons(current_theme)
            
        # 更新完成后，主动刷新一下菜单栏和整个界面
        self.menuBar().update()
        QApplication.processEvents()  # 强制处理所有未处理的事件
        ...
        if theme_name == "现代蓝":
            ...
            try:
                ...
                if os.path.exists(style_path):
                    ...
                    # 添加对列表项复选框的样式支持
                    list_checkbox_style = """
                    QListWidget::item::indicator:checked, QListView::item::indicator:checked {
                        image: url(checkmark_blue.png);
                    }
                    """
                    stylesheet += list_checkbox_style
            ...
            try:
                ...
                if os.path.exists(style_path):
                    ...
                    # 1. 清除所有字体相关设置
                    stylesheet = re.sub(r'font-size\s*:[^;]+;', '', stylesheet)
                    stylesheet = re.sub(r'font-family\s*:[^;]+;', '', stylesheet)
                    stylesheet = re.sub(r'font-weight\s*:[^;]+;', '', stylesheet)
                    ...
                    # 3. 应用特定控件样式覆盖（使用更高优先级）
                    specific_styles = """
                        QLineEdit, QPushButton, QComboBox, QLabel, QCheckBox, QRadioButton,
                        QTextBrowser, QTextEdit, QTreeView, QTableView, QTableWidget, QHeaderView,
                        QGroupBox, QTabBar, QTabWidget, QToolBar, QDockWidget, QDialog, QListWidget,
                        QScrollBar, QSlider, QSpinBox, QDoubleSpinBox, QDateEdit, QTimeEdit, QDateTimeEdit {
                            font-family: "Microsoft YaHei", "PingFang SC", "Segoe UI", Arial, sans-serif;
                            font-size: 9pt;
                            font-weight: normal;
                        }
                        
                        QMenuBar, QMenu, QToolTip, QStatusBar {
                            font-family: "Microsoft YaHei", "PingFang SC", "Segoe UI", Arial, sans-serif;
                            font-size: 9pt;
                            font-weight: normal;
                        }
                    ...
                    # 4. 先应用全局样式，再应用特定样式，最后应用原始样式表（确保正确的优先级）
                    stylesheet = global_font_style + specific_styles + stylesheet
                    ...
                    """
                    stylesheet = menu_style + stylesheet
                    ...
                    forced_inheritance = """
                        ...
                        * QWidget {
                        ...
                        /* 修复可能被原主题覆盖的对话框标题和按钮字体 */
                        QDialog QLabel, QDialog QPushButton, QDialog QCheckBox {
                            font-family: "Microsoft YaHei", "PingFang SC", "Segoe UI", Arial, sans-serif;
                            font-size: 9pt;
                        }
                    """
                ...
                print(f"Error applying modern green style: {e}. Falling back to system default.")
                app.setStyleSheet("")
            ...
            try:
                ...
                print(f"Error applying modern purple style: {e}. Falling back to system default.")
                app.setStyleSheet("")
        else:
            # 未知主题，回退到系统默认
            app.setStyleSheet("")
            print(f"Unknown theme: {theme_name}. Falling back to system default.")
...
class MainWindow(QMainWindow):  # Changed base class to QMainWindow
                ...
                if os.path.exists(style_path):
                    ...
                    forced_inheritance = """
                        ...
                        * QWidget {
                        ...
                        /* 修复设置对话框和列表中的字体 */
                        QSettings QLabel, QSettings QComboBox, 
                        QListWidget, QListWidget::item, QListView::item {
                            font-family: "Microsoft YaHei", "PingFang SC", "Segoe UI", Arial, sans-serif;
                            font-size: 9pt;
                        }
                    """
                    stylesheet = forced_inheritance + stylesheet
                    # -----------------------------------
                    ...
                    icon_css = """
                        ...
                        QCheckBox::indicator:checked {
                            image: url(checkbox_checked_green.png);
                        }
                        
                        QCheckBox::indicator:unchecked {
                            image: url(checkbox_unchecked_green.png);
                        }
                        ...
                        QRadioButton::indicator:unchecked {
                            image: url(radio_unchecked_green.png);
                        }
                    """
    ...
    def set_busy_state(self, is_busy):
        ...
        # 禁用或启用索引相关控件
        if hasattr(self, 'index_dirs_button'):
            self.index_dirs_button.setEnabled(not is_busy)
        if hasattr(self, 'index_button'):
            self.index_button.setEnabled(not is_busy)
        if hasattr(self, 'view_skipped_button'):
            self.view_skipped_button.setEnabled(not is_busy)
        
        # 禁用或启用过滤器控件
        if hasattr(self, 'file_type_checkboxes'):
    ...
    def start_indexing_slot(self):
        ...
        # 打印相关设置信息
        print(f"开始索引 {len(source_dirs)} 个源目录 -> '{index_dir}'。OCR: {enable_ocr}, 超时: {extraction_timeout}秒, TXT限制: {txt_content_limit_kb}KB")

        self.set_busy_state(True)
        self.results_text.clear()  # 清除之前的结果/日志
    ...
    def show_skipped_files_dialog_slot(self):
        """显示索引过程中被跳过的文件对话框"""
        try:
            if not hasattr(self, 'skipped_files_dialog'):
            ...
            print(f"显示跳过文件对话框出错: {e}")
            QMessageBox.warning(self, "错误", f"无法显示跳过的文件列表: {e}")
    ...
    def _update_feature_availability(self):
        """根据当前许可证状态启用或禁用特定功能"""
        # 从许可证管理器获取当前状态
        has_license = hasattr(self, 'license_manager') and self.license_manager.is_licensed()
        
        # 更新OCR选项的可用性
        if hasattr(self, '_ocr_action'):
            self._ocr_action.setEnabled(has_license)
        
        # 更新高级搜索功能的可用性
        if hasattr(self, 'advanced_search_button'):
            self.advanced_search_button.setEnabled(has_license)
        
        # 更新批量处理功能的可用性
        if hasattr(self, 'batch_process_button'):
            self.batch_process_button.setEnabled(has_license)
        
        # 更新Pro菜单
        self._update_pro_menu()
        
        # 更新界面主题
        theme = self.settings.value("gui/theme", "blue")
        if not has_license and theme != "blue":
            # Pro版本许可证无效时，强制使用蓝色主题
            self.settings.setValue("gui/theme", "blue")
    ...
    def _update_feature_availability(self):
        """根据当前许可证状态启用或禁用特定功能"""
        # 从许可证管理器获取当前状态
        has_license = hasattr(self, 'license_manager') and hasattr(self.license_manager, 'has_valid_license') and self.license_manager.has_valid_license()
    ...
    @Slot()
    def show_index_directories_slot(self):
        """显示索引目录选择对话框"""
        try:
            if not hasattr(self, 'index_directories_dialog'):
                self.index_directories_dialog = IndexDirectoriesDialog(self)
                # 连接目录选择变更信号
                self.index_directories_dialog.directoriesSelectionChanged.connect(
                    self._on_directories_selection_changed
                )
            
            self.index_directories_dialog.show()
            self.index_directories_dialog.raise_()
            self.index_directories_dialog.activateWindow()
        except Exception as e:
            print(f"显示索引目录对话框出错: {e}")
            QMessageBox.warning(self, "错误", f"无法显示索引目录选择界面: {e}")
    
    def _on_directories_selection_changed(self, selected_directories):
        """当用户在IndexDirectoriesDialog中选择的目录改变时触发"""
        print(f"已选择 {len(selected_directories)} 个索引目录")
        self.search_dirs = selected_directories
        # 如果当前有搜索关键词，可以自动刷新搜索结果
        if self.search_combo.currentText().strip():
            self._start_search_common("keyword", self.search_combo.currentText(), "selected_dirs")
    ...
    def _update_mode_buttons_state_slot(self, button=None, checked=None):
        """更新模式按钮的状态"""
        try:
            # 获取当前选择的搜索范围
            selected_scope = None
            for button in self.scope_button_group.buttons():
                if button.isChecked():
                    selected_scope = button.property("scope")
                    break
            
            # 根据搜索范围更新索引目录按钮状态
            if hasattr(self, 'index_dirs_button'):
                # 只有当选择了"选定目录"范围时，才启用索引目录按钮
                self.index_dirs_button.setEnabled(selected_scope == "selected_dirs")
            
            # 更新搜索框的提示文本
            if selected_scope == "all_dirs":
                self.search_combo.setPlaceholderText("在所有索引目录中搜索...")
            elif selected_scope == "selected_dirs":
                self.search_combo.setPlaceholderText("在选定的索引目录中搜索...")
            else:
                self.search_combo.setPlaceholderText("输入搜索关键词...")
            
            # 保存当前选择的搜索范围
            self.search_scope = selected_scope
            
        except Exception as e:
            print(f"更新模式按钮状态出错: {e}")
    ...
    def _update_theme_icons(self, theme_name):
        ...
        try:
            # 主题颜色
            color_suffix = ""
            if theme_name == "modern_blue" or theme_name == "blue":
                color_suffix = "blue"
            elif theme_name == "modern_green":
                color_suffix = "green"
            elif theme_name == "modern_purple":
                color_suffix = "purple"
            else:
                color_suffix = "blue"  # 默认使用蓝色

            # 更新下拉箭头图标
            arrow_icon_path = get_resource_path(f"down_arrow_{color_suffix}.png")
            print(f"资源路径解析: down_arrow_{color_suffix}.png -> {arrow_icon_path}")
            
            # 更新单选框图标
            radio_icon_path = get_resource_path(f"radio_checked_{color_suffix}.png")
            print(f"资源路径解析: radio_checked_{color_suffix}.png -> {radio_icon_path}")
            
            # 更新复选框图标
            checkbox_icon_path = get_resource_path(f"checkmark_{color_suffix}.png")
            print(f"资源路径解析: checkmark_{color_suffix}.png -> {checkbox_icon_path}")
            
            # 直接设置控件图标
            try:
                # 设置搜索下拉框的下拉箭头
                if hasattr(self, 'search_combo') and os.path.exists(arrow_icon_path):
                    self.search_combo.setStyleSheet(
                        f"QComboBox::down-arrow {{ image: url({arrow_icon_path}); }}"
                    )
                    print(f"已通过代码设置下拉箭头图标: {arrow_icon_path}")
                
                # 为其他控件设置图标
                if hasattr(self, '_apply_direct_arrow_icons'):
                    self._apply_direct_arrow_icons(color_suffix)
                
            except Exception as e:
                print(f"通过代码设置图标时发生错误: {e}")
            
        except Exception as e:
            print(f"更新主题图标时发生错误: {e}")
            
    def _apply_direct_arrow_icons(self, color_suffix):
        """直接为特定控件应用箭头图标"""
        try:
            # 获取资源路径
            arrow_icon_path = get_resource_path(f"down_arrow_{color_suffix}.png")
            ...
            if hasattr(self, 'sort_direction_button'):
                ...
                if self.sort_ascending:
                    # 使用向上箭头
                    up_arrow_path = get_resource_path(f"up_arrow_{color_suffix}.png")
                    if os.path.exists(up_arrow_path):
                        self.sort_direction_button.setIcon(QIcon(up_arrow_path))
                else:
                    # 使用向下箭头
                    if os.path.exists(arrow_icon_path):
                        self.sort_direction_button.setIcon(QIcon(arrow_icon_path))
            
        except Exception as e:
            print(f"直接应用箭头图标时发生错误: {e}")
...
class IndexDirectoriesDialog(QDialog):
    ...
    def _apply_theme_icons(self):
        ...
        try:
            ...
            if theme_name == "现代蓝" or theme_name == "系统默认":
                icon_path = get_resource_path("checkmark_blue.png")
                color = "#2980b9"
            elif theme_name == "现代绿":
                icon_path = get_resource_path("checkmark_green.png")
                color = "#27ae60"
            elif theme_name == "现代紫":
                icon_path = get_resource_path("checkmark_purple.png")
                color = "#8e44ad"
            else:
                # 默认使用蓝色
                icon_path = get_resource_path("checkmark_blue.png")
            ...
            print(f"已应用主题图标: {theme_name}, 图标路径: {icon_path}")
        except Exception as e:
            print(f"应用主题图标时出错: {e}")
            import traceback
            print(traceback.format_exc())
    ...
    def _toggle_all_directories(self, state):
        ...
        for i in range(count):
            ...
            item.setCheckState(check_state)  # 直接设置状态，不需要条件检查
